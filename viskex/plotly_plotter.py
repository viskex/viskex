...
class PlotlyPlotter(BasePlotter[  # type: ignore[no-any-unimported]
    ...
    color_sequence = plotly.colors.qualitative.Set1[:-1]
    nan_color = plotly.colors.qualitative.Set1[-1]
    cell_color = "blue"
    point_color = "blue"
    ...
    def plot_mesh(  # type: ignore[no-any-unimported, override]
        ...
        **kwargs: typing.Any  # noqa: ANN401
        ...
        fig.add_scatter(
            ...
            line=dict(color=cls.cell_color, width=2, dash="solid"),
            marker=dict(color=cls.point_color, symbol=symbol, size=10, line=dict(color=cls.point_color, width=2)),
            mode=mode
        )
    ...
    def plot_mesh_entities(  # type: ignore[no-any-unimported, override]
        ...
        assert len(all_values_unique) < len(cls.color_sequence), (
            "The color sequence only contains {len(cls.color_sequence)} colors + 1 color for NaN")
        colors = {
            value: cls.nan_color if value == int_nan else cls.color_sequence[value] for value in all_values_unique}
        fig = go.Figure()
        for value in all_values_unique:
            ...
            for s, coordinates_value_s in enumerate(coordinates_value):
                ...
                fig.add_scatter(
                    ...
                    marker=dict(color=marker_color, symbol=symbol, size=10, line=dict(color=marker_color, width=2)),
    ...
    def plot_scalar_field(  # type: ignore[no-any-unimported]
        ...
        fig.add_scatter(
            ...
            line=dict(color=cls.cell_color, width=2, dash="solid"),
            marker=dict(color=cls.point_color, symbol="circle", size=10, line=dict(color=cls.point_color, width=2)),
            mode="lines+markers"
        )
        fig.update_xaxes(title_text="x")
